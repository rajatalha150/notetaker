...
- **Deterministic Native Detection**: Before relying on manual hints, the desktop app attempts to derive platform and participant names from live native process/window metadata first, then source/window titles, using app-specific parsing rules.
...
- Desktop now performs deterministic participant detection from native process/window metadata and source titles, and combines that with local speaker-event monitoring.
- Native-app speaker-name detection is still less mature than the browser extension's DOM-driven detector when apps do not expose useful process/window metadata or titles, and that remains the main parity gap.
