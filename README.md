...
- **Speaker Diarization** — Automatically identifies speakers by name using real-time DOM scraping in browsers and stereo / AI-assisted patterns on Desktop.
...
Today the extension and desktop app share the same data model, AI pipeline, and settings UI, while persisting data in their own runtime-appropriate local stores. Desktop recordings are stored as real files managed by Electron and can be transcribed directly from the desktop library without manual file re-selection.
